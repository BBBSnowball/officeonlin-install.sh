...
apt-get install sudo curl libegl1-mesa-dev libkrb5-dev systemd python-polib git libkrb5-dev make openssl g++ libtool ccache libpng12-0 libpng12-dev libpcap0.8 libpcap0.8-dev libcunit1 libcunit1-dev libpng12-dev libcap-dev libtool m4 automake libcppunit-dev libcppunit-doc pkg-config wget libfontconfig1-dev graphviz xz-utils libcups2-dev openjdk-7-jdk gperf doxygen libxslt1-dev xsltproc libxml2-utils libxrender-dev libxrandr-dev bison flex zip libgtk-3-dev uuid-runtime libgl1-mesa-dev ant libssl-dev -y
[ $? -ne 0 ] && exit 1
...
"SO BE PATIENT PLEASE! ! You may see errors during the installation, just ignore them and let it do the work." 10 78
  ...
  {
  cd ${lo_dir}
  sudo -Hu lool ./autogen.sh --without-help --without-myspell-dicts --disable-gtk --disable-gstreamer-1-0 --disable-gltf --without-junit
  [ $? -ne 0 ] && exit 2
